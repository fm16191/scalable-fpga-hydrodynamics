...
def render_csv_to_png(src: str, out_dir: str, var: str, vmin: float, vmax: float):
    srcp = Path(src)
    ...
    out_path = out_dir / f"frame_{it:05d}_{ts:.6f}.png"

    df = pd.read_csv(srcp)
    if not {"x", "y", var} <= set(df.columns):
        return None

    grid = df.pivot(index="y", columns="x", values=var)
    ...
    ax.set_title(f"Hydrodynamics : {var} at timestep {ts:.4e} (iteration={it})")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_xticks([])
    ax.set_yticks([])
...
def main():
    p = argparse.ArgumentParser(description="Render CSV grid frames to PNG.")
    ...
    with ProcessPoolExecutor(max_workers=args.jobs) as ex:
        results = ex.map(render_csv_to_png, map(str, sorted_files), repeat(args.out), repeat(args.var), repeat(vmin), repeat(vmax))
