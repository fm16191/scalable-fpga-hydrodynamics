...
static int print_usage(char *exec)
{
    ...
    printf("Options : \n"
        ...
        NB_SUBDOMAINS_X, NB_SUBDOMAINS_Y, NB_X, NB_Y, MAX_T, out_filename.c_str());
    return 0;
}

static void write_results(const vector<vector<sub_domain>> &subdomains, const size_t iteration,
                          const double timestep)
{
    ...
    for (size_t sd_i = 0; sd_i < NB_SUBDOMAINS_X; ++sd_i) {
        for (size_t sd_j = 0; sd_j < NB_SUBDOMAINS_Y; ++sd_j) {
            ...
            const sub_domain &cur = subdomains[sd_i][sd_j];
    ...
    const std::string header = "x,y,rho,p,u,v\n";
...
static double compute_total_mass(const vector<vector<sub_domain>> &subdomains)
{
...
static double ensure_mass_conservation(double initial_mass, const vector<vector<sub_domain>> &subdomains,
...
static void update_ghost_cells(vector<vector<sub_domain>> &subdomains, it_timers_t &T)
{
    ...
    sub_domain &cur = subdomains[rank][0];
    const size_t y_stride = cur.y_stride;
...
int main(int argc, char *argv[])
{
    ...
    const DATATYPE min_spacing = std::min({ DX, DY });
    ...
    vector<vector<sub_domain>> subdomains; // Rank 0 only
    ...
    // Create a 2x2 grid of subdomains - INITIALISATION
    SUBDOMAIN_SIZE_X = NB_X / NB_SUBDOMAINS_X;
    SUBDOMAIN_SIZE_Y = NB_Y / NB_SUBDOMAINS_Y;

    for (size_t sd_i = 0; sd_i < NB_SUBDOMAINS_X; ++sd_i) {
        vector<sub_domain> row;
        row.reserve(NB_SUBDOMAINS_Y);
        for (size_t sd_j = 0; sd_j < NB_SUBDOMAINS_Y; ++sd_j) {
            size_t sd_index = sd_i * NB_SUBDOMAINS_Y + sd_j;
            if (rank != sd_index % nprocs) { row.emplace_back(queue); continue; }
    ...
    while (t < MAX_T && it < MAX_IT) {
        ...
        it_timers_t T_total;
        static_assert(sizeof(it_timers_t) == sizeof(double) * 7, "Timers struct must be packed with 7 doubles.");
    ...
    if (rank == 0) {
        ...
        const double problem_size = double(NB_X*NB_Y);
        const unsigned long boundary_problem_size_x = 2*NB_X;
        const unsigned long boundary_problem_size_y = 2*NB_Y;
        ...
        printf(" %5.1lf %% (%.2lf s) Copies (%2.1lf %% CPU→FPGA + %2.1lf %% FPGA→CPU) (%.2lf s + %.2lf s)\n",
               copies_total_s * pt, copies_total_s / 1e6, res_host_to_device.mean * pt,
               res_device_to_host.mean * pt, res_host_to_device.mean / 1e6, res_device_to_host.mean / 1e6);
        double boundaries_total_s = res_boundaries_x.mean + res_boundaries_y.mean;
        printf(" %5.1lf %% (%.2lf s) Boundary Conditions (%2.1lf %% BC X + %2.1lf %% BC Y) (%.2lf s + %.2lf s)\n",
               boundaries_total_s * pt, boundaries_total_s / 1e6, res_boundaries_y.mean * pt,
               res_boundaries_x.mean * pt, res_boundaries_y.mean / 1e6, res_boundaries_x.mean / 1e6);

        printf("\n");
        printf("Subdomains : [%lu][%lu] = %lu\n", NB_SUBDOMAINS_X, NB_SUBDOMAINS_Y, NB_SUBDOMAINS_X * NB_SUBDOMAINS_Y);
